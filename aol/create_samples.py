...
from lib import read_volume_data
...
def sample(click_files, volume_file, target_file, num_users, num_clicks):
	print "Sampling users."
	users = select_users(volume_file, num_users, num_clicks)
	
	print "Reading user clicks."
	clicks = read_users_data(click_files, users)
...
def main():
	tweet_files = glob.glob(os.path.join(os.getenv("TD"), "aol", "news-only", "*"))
	users_volumes_file = os.path.join(os.getenv("TR"), "aol", "aol-user-volumes-news-only.txt")

	# (num users, num clicks per user)
	sample_params = [(150, 100)]
	for num_users, num_clicks in sample_params:
		print "Generating sample with %d users and %d clicks" % (num_users, num_clicks)
		target_file = os.path.join(os.getenv("TD"), "aol", "sample-u%d-c%d-news-only.txt" % (num_users, num_clicks))
